alter table public.order_items
  add column if not exists print_paused boolean not null default false;
...
begin
  ...
  loop
    ...
    values (
      ...
      case when jsonb_typeof(v_line -> 'printPaused') = 'boolean' then (v_line ->> 'printPaused')::boolean else false end
    );
  end loop;
