...
export const posKnowledgeArticles: PosKnowledgeArticle[] = [
  ...
  },
  {
    ...
    steps: [
      '在點餐頁付款快捷或「付款/拆單」中，可選現金、刷卡、自定義支付、LINE Pay 或街口。',
      '刷卡可用於未整合刷卡機的信用卡記錄；自定義支付用於禮券、兌換券等基本支付記錄，不會出現在消費者線上點餐付款方式。',
      '付款頁的支付備註可填禮券號碼、外送平台或特殊收款資訊，送單後會保存在訂單並可於交易查詢使用「支付備註」篩選。',
      '若同張單有多種付款來源，開啟混合支付後可把其中一筆付款方式設為自定義支付。',
      '送單後付款方式會寫入 Supabase 訂單欄位、列印 payload 與營運日報；支付備註會寫入 `orders.payment_note`，換平板或 fresh reinstall 後仍可還原。',
    ],
    keywords: ['基本支付', '自定義支付', '支付備註', '禮券', '兌換券', '現金', '刷卡', '混合支付', 'iCHEF'],
    target: 'payment',
  },
  {
    id: 'split-bills',
  ...
  {
    id: 'checkout-void-record-report',
    category: 'reports',
    title: '結帳／作廢紀錄',
    summary: '後台營運報表可依 iCHEF 結帳作廢紀錄查詢每筆結帳、收據/發票、付款資訊、帳本與作廢/退款狀態。',
    steps: [
      '連點工具箱 6 下進入後台，再到「營運報表」。',
      '在「結帳／作廢紀錄」選擇開始/結束日期；單次查詢不可超過 93 天，資料窗口限近 2 年。',
      '可依目前概況、服務方式、訂單來源與人數範圍篩選；目前概況可切到已開立、已作廢、已退款或已註銷。',
      '摘要會顯示收據/發票結帳金額、服務費、運費、折扣/折讓、已開立張數與作廢/退款/註銷張數。',
      '列表會顯示發票或收據號碼、結帳時間、原始/外部單號、訂單來源與種類、服務費、運費、折扣、結帳金額、支付模組、帳本、付款資訊、支付備註、顧客與訂購人資訊。',
      '按「匯出紀錄 CSV」可下載目前篩選結果；結帳時間優先使用付款完成時間，沒有付款流水時回退訂單狀態更新時間。',
    ],
    keywords: ['結帳作廢紀錄', '結帳紀錄', '作廢註銷記錄', '收據', '發票', '付款資訊', '支付備註', '帳本', 'CSV', '報表分析', 'iCHEF'],
    target: 'admin',
  },
  {
  ...
  {
    id: 'discount-analysis-report',
    category: 'reports',
    title: '優惠活動分析',
    summary: '後台營運報表可依 iCHEF 優惠活動分析查看優惠訂單營業額、折抵金額、訂單數與活動明細。',
    steps: [
      '連點工具箱 6 下進入後台，再到「營運報表」。',
      '在「優惠活動分析」選擇開始/結束日期；單次查詢不可超過 93 天，資料窗口限近 2 年。',
      '可用日、週、月切換成效總覽單位，也可依服務方式、訂單來源與人數範圍篩選。',
      '摘要會顯示營業淨額、優惠訂單營業額、優惠折抵金額、優惠活動訂單數與總營業額占比。',
      '優惠列表會依店家優惠、優惠券彙總活動名稱、類型、優惠活動營業額、小計、折抵金額與訂單數；通路小計分成 POS、線上與掃碼。',
      '按「匯出優惠 CSV」可下載目前篩選結果；自動優惠名稱優先讀建單稽核，優惠券名稱優先讀兌換紀錄。',
    ],
    keywords: ['優惠活動分析', '店家優惠', '優惠券', '優惠折抵金額', '優惠訂單', '營業額占比', 'CSV', '報表分析', 'iCHEF'],
    target: 'admin',
  },
  {
  ...
  },
  {
    ...
    category: 'orders',
    title: '標籤管理與備註可見性',
    summary: '工具箱「標籤管理」可維護店內常用服務標籤，並把消費者備註與 POS 店內備註分開保存。',
    steps: [
      ...
      'POS 點餐頁輸入的備註會保存為店內備註；雲端餐廳或 QR 消費者輸入的備註會保存為消費者備註。',
      '顧客聯、請款明細與交易明細只使用消費者備註，避免店員服務記錄出現在消費者單據上。',
      '訂單送出後標籤與備註會寫入訂單欄位，POS、外帶/外送清單與後台紀錄都能追查，fresh reinstall 後仍從資料庫還原。',
    ],
    keywords: ['標籤', '標籤管理', '訂單標籤', '服務標籤', '備註', '店內備註', '消費者備註', 'POS', '後台紀錄'],
    target: 'order',
  },
  {
    ...
    category: 'orders',
    title: '顧客資訊管理',
    summary: '工具箱「顧客資訊管理」可搜尋顧客、查看消費分析、依類型篩選、切換排序並新增顧客資料。',
    steps: [
      '開啟工具箱「顧客資訊管理」，進入後台編輯模式後按「刷新顧客」載入資料。',
      '可用姓名、電話、LINE UID 或顧客類型搜尋，也可用顧客類型篩選。',
      '排序可切換為最近消費時間或建立時間；最近消費時間優先使用完成收款訂單分析，沒有消費分析時才退回會員流水或更新時間。',
      '列表會顯示會員消費次數、累積金額、點數、錢包與點選率排行，資料由 `/admin/members` 即時計算。',
      '新增顧客時可填姓名、電話、顧客類型與開通點數，資料會寫入 `members` 與既有會員 API。',
      '點餐頁 CRM 搜尋、後台會員錢包與工具箱都讀同一份顧客資料，換平板或 fresh reinstall 後仍可查。',
    ],
    keywords: ['顧客資訊', '會員', 'CRM', '顧客類型', '搜尋顧客', '新增顧客', '會員分析', '點選率排行'],
    target: 'order',
  },
  {
    id: 'member-sales-analysis',
    category: 'orders',
    title: '會員消費分析',
    summary: '點餐頁搜尋會員後會顯示消費次數、金額、平均週期、上次消費日、點選率排行、點數與優惠券。',
    steps: [
      '在點餐頁顧客查詢輸入手機、姓名或 LINE UID，選擇搜尋結果中的會員。',
      '會員分析面板會顯示消費次數、累積消費金額、平均消費金額、平均消費週期與上次消費日。',
      '點選率排行會從已完成收款的會員訂單明細彙整前 5 名商品，協助櫃台快速掌握常點品項。',
      '同一份分析也會出現在工具箱「顧客資訊管理」，點數與優惠券仍讀會員餘額與 `member_coupons`。',
      '分析由 `pos-api` 的 `/members/search` 與 `/admin/members` 即時計算，不另存 localStorage 或快取表。',
    ],
    keywords: ['會員分析', '會員喜好', '點選率排行', '上次消費日', '消費次數', 'CRM', 'iCHEF'],
    target: 'order',
  },
  {
