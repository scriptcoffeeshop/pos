...
- 建立後台商品管理入口，先支援前台商品顯示、價格、排序與停售；目前前端以工具箱連點 6 下進入後台編輯模式。
...
- 依後台印單規則決定貼紙/收據、品項分類、指定品項與出單機；未納入規則的品項不列印。已接入前台列印計畫與多筆 `print_jobs`，待實機校正 EZPL 版面。
...
- 未收款訂單作廢已接入 POS 佇列與下一張單，前端需後台編輯模式，會排除關帳/班別統計；已收款訂單退款已接 `refund_pos_order()`，會同步寫入負數交易流水。
- 正式班別/收銀機 session 已接第一版：`register_sessions` 保存開班/關班、開班現金、實點現金、預期現金、現金銷售、非現金、待收款、單數與關帳異常計數；開關班需後台編輯模式。
- 營運日報已接第一版：後台可查詢指定日期的實收、待收、退款、異常、付款方式、來源、服務方式、時段與熱門商品。
...
- 供應狀態常駐：商品暫停/恢復入口從 Native-only 改成 POS 列印/前台操作頁固定可見，改由工具箱連點 6 下的後台編輯模式保護。
...
- 訂單任務快篩列：POS 佇列新增到點/逾時、待收款、可交付、線上未確認與列印失敗任務卡，點擊即可套用對應查詢條件。
- iPad 截圖主流程重塑：2026-05-03 依使用者提供的 6 張 iPad 截圖先落地主畫面、右側工具箱入口、新增外帶雙欄點單、飲品選項面板、飲品溫度必填警告、送出後回佇列，以及訂單滑動揭露「已完成/已取餐」與「作廢/退款」。主佇列第一層同步收斂為搜尋、條件與訂單列，任務卡與履約警示後續改適合放入工具箱或次層操作。

### 後續評估

- 門市知識庫內容擴充：目前已內建操作流程；未來可匯入真實 SOP、新人訓練文件或影片索引。
- 更完整的背景通知與提示模式：目前已支援前景提示音；背景播放、音量控制與系統通知需先確認平板瀏覽器/APK 限制。
