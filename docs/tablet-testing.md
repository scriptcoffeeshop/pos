...
3. 平板 B 重新同步後點同一套餐商品，應開啟套餐面板；未選必選子項目時不得加入訂單，選擇後票券應顯示子項目名稱與加價後單價。
4. 用消費者線上點餐或 QR 頁點同一套餐，確認同一組子項目規則、重複選擇限制與加價一致。
5. 正式出單或結帳後，在另一台平板展開訂單，確認套餐子商品仍從 `order_items.combo_items` 還原，不只存在送單當下的本機記憶體。
6. 在列印站設定只列印某個套餐子商品或把它列入不計算商品，建立含該子商品的套餐後，列印 preview / `print_jobs` 應依子商品命中規則。
7. 若套餐子商品有可售庫存或自動耗料規則，正式建單後應扣子商品庫存與 `inventory_records.consumption`；將主商品明細切成「暫停出單」時不應扣套餐子商品耗料。
8. fresh reinstall APK 或清除瀏覽器資料後重新進入 POS，確認套餐設定、已送出的套餐子商品、列印規則與庫存紀錄仍從 Supabase 還原。
