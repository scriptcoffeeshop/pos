...
4. 平板 B 將該訂位改為取消或未出席；平板 A 重新整理後應看到同步狀態。
5. 建立同桌同時段或座位不足的測試資料，確認 POS 訂位管理出現「桌位重疊」或「座位數不足」提示。
6. 將可入座的訂位按「帶位開單」，確認 POS 進入內用點餐草稿，顧客姓名、電話、訂位人數、備註與保留桌位被帶入，且另一台平板看到訂位狀態為 `seated`。
7. fresh reinstall APK 或清除瀏覽器資料後重新進入 POS，確認訂位狀態、修改後資訊、桌位安排與桌位圖下一組訂位時間仍從 Supabase 還原。
