...
內用掃碼結帳模式存在 `online_ordering.dineInCheckout`，對照 iCHEF 後台「內用掃碼點餐 > 結帳流程」的「先結 / 後結」。測試時至少準備一個後台工作站、一個 POS 平板與一個 QR/消費者頁：

1. 在平板 A 連點工具箱 6 下進入後台編輯模式，到後台「線上點餐」將「內用掃碼結帳模式」切成「後結」，填入最多 500 字的結帳說明後儲存。
2. 在平板 B 建立內用桌位單並列印或預覽訂單 QR Code，用 QR 頁開啟點餐，確認結帳區顯示後結說明且不顯示付款方式按鈕。
3. 後結模式送出 QR 內用單後，平板 A 與平板 B 都應看到該單以現場待收款進 POS；繞過前端送單時，`pos-api` 也應把付款方式覆寫成 `cash`、付款狀態覆寫成 `pending`。
4. 在平板 A 切成「先結」後儲存，平板 B 或 QR 頁重新整理後應只顯示 LINE Pay、街口與線上刷卡等線上付款方式，不顯示取餐時付款或轉帳。
5. 先結模式下嘗試用舊頁或 API 送出 `cash` / `bank_transfer`，`POST /orders` 應回覆 409，不得寫入訂單。
6. fresh reinstall APK 或清除瀏覽器資料後重新進入 POS，確認結帳模式、結帳說明與 QR 頁付款行為仍從 Supabase runtime 還原。

## 多平板線上點餐備註欄位

餐點備註與訂單備註設定存在 `online_ordering.commentFields`，對照 iCHEF 後台「內用掃碼點餐 / 外帶外送訂餐 > 備註欄位」。測試時至少準備一個後台工作站、一個 POS 平板與一個 QR/消費者頁：

1. 在平板 A 連點工具箱 6 下進入後台編輯模式，到後台「線上點餐」設定餐點備註為顯示、訂單備註為選填，並修改備註提示文字後儲存。
2. 在平板 B 或 QR/消費者頁重新整理，加入一個品項，確認品項選項面板可輸入「餐點備註」，送單後平板 A 的待接單明細與訂單中心都應看到該文字註記。
3. 平板 A 將訂單備註改成「必填」後儲存，平板 B 重新整理後不填備註不得送單；填寫後送單應同步到 POS 訂單備註。
4. 平板 A 將訂單備註改成「隱藏」後儲存，平板 B 重新整理後整筆訂單備註欄位應消失；QR 內用單仍保留桌位資訊，舊頁或 API 送出的額外備註不應寫入 `orders.note`。
...
用餐限時與最後加點規則存在 `online_ordering.dineInTimeLimit`，QR URL 會帶開單時間，公開 API 也會重新驗證最後加點時間。測試時至少準備一個後台工作站、一個 POS 平板與一個 QR/消費者頁：

1. 在平板 A 連點工具箱 6 下進入後台編輯模式，到後台「線上點餐」開啟「用餐與點餐限時」，設定用餐限時與最後加點分鐘，必要時新增假日規則。
2. 在 POS 工具箱按「用餐與點餐限時」卡片切換開關，確認後台重新載入後看到同一個功能狀態。
3. 建立內用桌位單並列印或預覽訂單 QR Code，確認 QR URL 含 `source=qr`、`order`、桌號與 `openedAt`。
4. 用 QR 頁開啟點餐，確認頁面顯示最後加點與用餐結束時間；未到最後加點前可送單。
5. 將規則調成已超過最後加點，或用測試 URL 把 `openedAt` 改成足夠早的時間，確認 QR 頁停用加入/送出且 `POST /orders` 回 409，不得只靠前端阻擋。
6. 平板 A 桌位地圖應顯示剩餘或逾時時間；用餐結束後桌卡出現鬧鐘式警示，平板 B 重新同步後看到同一桌位時間狀態。
7. fresh reinstall APK 或清除瀏覽器資料後重新進入 POS，確認限時規則、工具箱開關與既有桌位時間都從 Supabase runtime / 訂單時間還原。
...
印單時機測試：在列印站或後台出單規則把一條規則只保留「出單」或只保留「重印」並儲存。新建訂單初次出單時只應套用「出單」規則；訂單列顯示「重印」後再列印時只應套用「重印」規則。換另一台平板或 fresh reinstall 後，`printer_settings.rules[].timings` 應仍由 Supabase runtime 還原。
