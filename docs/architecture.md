...
- Web POS 與 Capacitor Android APK 共用同一套 Vue build；APK 專案已在 `android/`，debug APK 由 GitHub Actions artifact 或本機 Gradle 產出。
- 所有互動都由 Vue component event 與 composable 管理。
- 禁止新增 inline event handler、`data-action` bridge、`window.*` API 與手動 `innerHTML` 資料渲染。
- 前台與後台共用同一個 Vue App，透過工作區切換進入 POS、商品菜單、出單規則與權限管理；後台寫入必須經 `POS_ADMIN_PIN` 保護的 Edge Function。
...
- `pos-api` 對公開前台提供商品、訂單與 runtime 出單機設定端點，對後台提供 PIN 保護的商品與設定端點。
