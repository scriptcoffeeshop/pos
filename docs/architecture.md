...
- `usePosSession()` 訂閱 `orders`、`runtime_settings`、`register_sessions` 與 `products` topic；收到事件會 debounce 後重新呼叫 `/orders`、`/settings/runtime`、`/register/current` 或 `/products`。POS 商品列表以 `pos_visible` 為同步邊界，因此 `is_available=false` 的停售品項仍會回到前端反灰顯示並可從供應狀態恢復；線上/QR 商品查詢仍由 API 過濾不可售品項。工具箱標籤管理寫入 `engagement_settings.orderLabels`，再經同一個 `runtime_settings` invalidation 讓點餐標籤列更新；訂單實際勾選結果仍保存在 `orders.order_labels`。若正在建單、出單、claim、收款、作廢或退款，訂單同步會延後重試，避免背景更新踩到操作中的狀態。
