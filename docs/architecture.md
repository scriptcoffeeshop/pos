...
- POS 工作台會訂閱 Supabase Realtime invalidation event，讓訂單佇列、runtime 設定、收銀班別、商品供應與庫存管理變更先由事件觸發重新載入；每 20 秒短輪詢仍保留為斷線 fallback，平板回到前景時也會立即補同步一次，避免多平板 claim lease 釋放或逾時後仍顯示舊狀態。Android APK 背景或熄屏後由 `OnlineOrderNotifier` native plugin 啟動 `OnlineOrderPollingService` foreground service，依 `online_ordering` runtime 短輪詢 `/settings/runtime`、`/orders` 與 `/online-order-reminders/state`，用系統 notification 和原生 tone 補上線上/掃碼新單提醒；Web 背景只提供 Browser Notification API fallback。API 失敗建立的櫃台單會先存在平板本機，後續同步成功時補寫 Supabase 並以訂單編號去重。
...
- `pos_realtime_events` 是前端唯一直接訂閱的 Realtime 表；`orders`、`pos_settings`、`register_sessions`、`products` 與庫存管理/自動消耗規則表 trigger 只寫入低敏感 topic/event/entity payload，POS 與線上點餐收到事件後仍回到 `pos-api` 取得正式資料，不直接訂閱受保護的來源表。
...
- iCHEF 式庫存管理另外使用 `inventory_categories`、`inventory_items`、`inventory_records` 與 `inventory_consumption_rules` 保存原料/包材類別、品項、操作紀錄與商品/註記自動消耗設定；`apply_inventory_record()` 以 row lock 在同一 transaction 內處理進貨、退貨、消耗、報廢與盤點，正式建單/出單時的自動消耗也會寫入 `inventory_records.consumption`，避免和可售商品庫存欄位混在一起。
...
- `pos_audit_events` 保存 POS 關鍵操作事件；建單、claim、釋放、狀態更新、收款、付款逾期、退款、作廢、商品/設定異動、庫存管理、自動消耗規則、會員建立、錢包調整、開班與關班都由 Edge Function 以 service role 寫入。商品稽核會附上庫存、低庫存門檻、售價、上下架與暫停供應的前後值/差額，庫存管理稽核會附上操作類型與操作後存量；後台透過 `/admin/audit-events` 讀取，前端不能直接改。
...
- Capacitor native plugins：Android APK 內的 `LanPrinter` native plugin 直接連線出單機 IP，送出 EZPL；`OnlineOrderNotifier` native plugin 搭配 `OnlineOrderPollingService` foreground service 在背景處理線上/掃碼新單 notification、提示音、稍後提醒與已讀同步。GitHub Pages 瀏覽器版只做預覽、雲端 print job 與 Browser Notification fallback。消費者線上點餐只以 Web 形式提供，不包進 APK 操作介面。
...
- `usePosSession()` 訂閱 `orders`、`runtime_settings`、`register_sessions`、`products`、`online_order_reminders`、`cash_drawer` 與 `inventory_management` topic；收到事件會 debounce 後重新呼叫 `/orders`、`/settings/runtime`、`/register/current`、`/products`、提醒 state、錢櫃事件，或在庫存面板開啟時觸發 `/admin/inventory` reload。POS 商品列表以 `pos_visible` 為同步邊界，因此 `is_available=false` 的停售品項仍會回到前端反灰顯示並可從供應狀態恢復；線上/QR 商品查詢仍由 API 過濾不可售品項。工具箱標籤管理寫入 `engagement_settings.orderLabels`，再經同一個 `runtime_settings` invalidation 讓點餐標籤列更新；訂單實際勾選結果仍保存在 `orders.order_labels`。若正在建單、出單、claim、收款、作廢或退款，訂單同步會延後重試，避免背景更新踩到操作中的狀態。
