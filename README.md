...
- 前端 POS API 同步：商品、訂單、runtime 設定與收銀班別從 `pos-api` 載入，櫃台建單、付款狀態、退款沖銷、訂單狀態、訂單 claim lease、列印工作與開關班寫回 Supabase；POS 會每 20 秒自動同步訂單佇列與班別摘要，API 失敗時保留本機 fallback。
- 後台入口：可切換到「後台」管理商品菜單、庫存數量、低庫存門檻、暫停供應至、POS/線上/掃碼可見性、備餐站、出單機與印單規則、角色權限與操作稽核；遠端讀寫需設定 `POS_ADMIN_PIN`。
