...
- 門市 POS 第一屏：品項、購物車、付款方式、顧客備註、訂單佇列；已加入最近/常用品項快速加購、常用備註 chip、佇列篩選、庫存/低庫存提示、下單原子扣庫存、訂單平板鎖定、收款確認、未收款訂單作廢、已收款退款沖銷、列印重印紀錄、正式開班/關班、關帳異常檢查與強制確認、鍵盤捷徑（`/` 搜尋、`1`-`6` 加購、`Cmd/Ctrl+Enter` 建單）。
- 消費者線上點餐頁：依後台「線上」通路顯示菜單，支援購物車、取餐方式、顧客資料與線上建單。
- 前端 POS API 同步：商品、訂單、runtime 設定與收銀班別從 `pos-api` 載入，櫃台建單與扣庫存、付款狀態、退款沖銷、訂單狀態、訂單 claim lease、列印工作、平板心跳與開關班寫回 Supabase；POS 會每 20 秒自動同步訂單佇列與班別摘要，API 失敗時保留本機 fallback。
- 後台入口：可切換到「後台」管理商品菜單、庫存數量、低庫存門檻、暫停供應至、POS/線上/掃碼可見性、備餐站、出單機與印單規則、角色權限、平板在線與操作稽核；遠端讀寫需設定 `POS_ADMIN_PIN`。
...
- 操作稽核：建單、收款、退款、作廢、訂單狀態、平板鎖定、商品/設定異動與開關班會寫入 `pos_audit_events`，商品稽核會標出庫存/售價等欄位的異動前後與差額，後台可依 PIN 讀取最近紀錄追查。
