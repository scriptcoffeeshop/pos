...
- 影片參考導入：在不改預設色系的前提下，補上 iCHEF 式工具箱、外觀偏好、訂單查詢進階條件（日期、取餐方式、來源、排序）與常駐供應狀態入口；供應狀態支援搜尋、分類、狀態篩選與批次暫停/恢復，後台營運紀錄會整合平板心跳、開關班與關鍵操作時間線，這些工作站偏好會保存於本機，適合平板反覆操作。
...
- 後台入口：可切換到「後台」管理商品菜單、庫存數量、低庫存門檻、暫停供應至、POS/線上/掃碼可見性、備餐站、會員錢包與 CSV 匯出、營運日報與 CSV 匯出、金流回呼事件篩選/匯出、出單機與印單規則、角色權限、營運紀錄、平板在線與操作稽核匯出；遠端讀寫需設定 `POS_ADMIN_PIN`。
