...
- 門市 POS 第一屏：品項、購物車、付款方式、顧客備註、訂單佇列；已加入最近/常用品項快速加購、常用備註 chip、佇列篩選、庫存/低庫存提示、訂單平板鎖定、列印重印紀錄、關帳摘要與鍵盤捷徑（`/` 搜尋、`1`-`6` 加購、`Cmd/Ctrl+Enter` 建單）。
- 消費者線上點餐頁：依後台「線上」通路顯示菜單，支援購物車、取餐方式、顧客資料與線上建單。
- 前端 POS API 同步：商品與訂單從 `pos-api` 載入，櫃台建單、狀態更新、訂單 claim lease 與列印工作寫回 Supabase；API 失敗時保留本機 fallback。
