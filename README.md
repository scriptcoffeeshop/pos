...
- 後台入口：連點工具箱 6 下會進入後台編輯模式，可切換到「後台」管理商品菜單、線上點餐 runtime 設定、庫存數量、低庫存門檻、暫停供應至、POS/線上/掃碼可見性、備餐站、會員錢包與 CSV 匯出、營運日報與 CSV 匯出、金流回呼事件篩選/匯出、出單機與印單規則、角色權限、營運紀錄、平板在線與操作稽核匯出；遠端商品新增/刪除/修改由 `pos-api` 直接寫入資料庫並記錄稽核。
