...
- 影片參考導入：在不改預設色系的前提下，補上 iCHEF 式工具箱、外觀偏好、訂單查詢進階條件（日期、取餐方式、來源、排序）與常駐供應狀態入口；供應狀態支援搜尋、分類、狀態篩選與批次暫停/恢復，後台營運紀錄會整合平板心跳、開關班與關鍵操作時間線，線上點餐設定可控制接單、預約、平均備餐時間、未確認提醒與提示音，POS 佇列會標示逾時未確認的線上/掃碼新單並在前景播放短提示音，工具箱內建可搜尋的門市 SOP 助手，這些工作站偏好會保存於本機，適合平板反覆操作。
