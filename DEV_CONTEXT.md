...
- 線上提醒共享狀態：`20260509023000_add_online_order_reminder_states.sql` 新增 `online_order_reminder_states`，並把 `pos_realtime_events.topic` 擴充為可接 `online_order_reminders`。`pos-api` 新增 `GET/PATCH/POST /online-order-reminders/state`，前端 `src/lib/posApi.ts` 會以訂單編號查詢或寫入 `snooze`、`seen`、`accepted`、`rejected`；`usePosSession()` 在套用遠端訂單前先同步 reminder states，並在收到 `online_order_reminders` event 時只重拉提醒狀態。Android `OnlineOrderNotifierPlugin` 背景輪詢 `/orders` 後會再查同一個 state endpoint，native snooze/markSeen 也會 POST 回 API，所以 foreground banner、Web fallback 與 native notification 使用同一個 suppression source。這個狀態是跨平板、跨 fresh reinstall 的來源，localStorage 只保留 station id 與既有 fallback 快取，不再承擔提醒去重。
