...
- 初始畫面：門市 POS 工作台預設進入簡潔的「外帶 / 外送」訂單查詢主畫面，主導航固定在右側並切回上一版綠色主色系；主畫面可直接搜尋/篩選訂單，並用「新增外帶」進入點餐。點餐頁仍保留分類欄、商品大按鈕矩陣與右側票券/結帳三欄；商品方格與 Cart 品項可直接輸入數量並保留正負號調整；票券上可直接修改顧客、方式、付款與常用備註；`order.scriptcoffee.com.tw` 會預設進入消費者線上點餐頁。
