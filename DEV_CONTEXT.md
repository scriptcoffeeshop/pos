...
- 列印計畫：`src/lib/printing.ts` 會依 `printer_settings` 的服務方式、品項分類、指定品項、貼紙/收據模式與 copies 建立多筆 EZPL payload；未被印單規則的分類或品項納入時不會列印。前台列印站已整理為 iCHEF 式出單機清單、印單規則與列印佇列，印單規則可在列印站直接勾選分類與品項，底部「列印流程」提示已移除，不再承擔庫存/供應管理。分類 row 的名稱點擊只切換右側品項列表，checkbox 才改變整類選取；整類選取時右側該分類所有品項都顯示勾選，若取消單一品項，前端會移除 `rule.categories` 的該分類並把同分類其他品項寫進 `rule.itemIds`，避免 category OR itemIds 的列印判斷和畫面 checkbox 狀態錯位。內用與外送舊收據規則載入時會正規化為「內用貼紙」「外送貼紙」。列印站 healthcheck 會分離畫面預覽與實際 EZPL payload，`usePosSession()` 逐筆建立/回寫 print job，列印頁會列出 print jobs 並可滑動刪除，Android APK 逐筆送 TCP。
