...
- 初始畫面：門市 POS 工作台預設進入接近 iCHEF Server 的「桌位地圖」，固定右側導航列已移除，工具箱改為可自由拖曳的浮動入口，並保留上一版綠色主色系。桌位地圖提供內用與外帶/外送第一層切換，內建 A1/A2/A3 桌卡、空桌/進行中/可交付/鎖定狀態、人數、金額、未出單等待、桌內滯留、候位清單、桌位顯示設定與通知中心；選桌只會選取桌位，右側控制區才會開桌點餐、開啟訂單或清桌，避免誤觸建立訂單。外帶/外送頁保留搜尋/篩選訂單，並用「新增外帶」進入點餐。點餐頁改為接近 iCHEF 出單畫面的左側票券、右側分類列與商品大方塊，新增外帶會立即建立可見佇列訂單，依序顯示 `No. 001`、`No. 002`，即使尚未加入品項也會保留在外帶/外送頁；內用訂單以 `桌位 A1` 這類備註對應桌卡，暫不新增資料庫 schema。咖啡/茶飲品項會開啟選項面板並要求先選「飲品溫度」，未選時會顯示必填警告；票券上可直接修改顧客、方式、付款與常用備註；`order.scriptcoffee.com.tw` 會預設進入消費者線上點餐頁。
